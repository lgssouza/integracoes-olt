...
class Fiberhome():
    ...
    def autorizaOnu(ip_olt, numero_pon, mac_onu, tipo_onu, nome_cliente, vlan):
        resposta    = [] 
        script_liberacao_onu = 'ADD-ONU::OLTID='+ip_olt+',PONID=NA-NA-'+numero_pon+':CTAG::AUTHTYPE=MAC,ONUID='+mac_onu+',NAME='+nome_cliente+',ONUTYPE='+tipo_onu+';CFG-LANPORTVLAN::OLTID='+ip_olt+',PONID=NA-NA-'+numero_pon+',ONUIDTYPE=MAC,ONUID='+mac_onu+',ONUPORT=NA-NA-NA-1:CTAG::CVLAN='+vlan+';'
        ...
        for cliente in clientes:
            ...
            while (resp_desautoriza != "Sucesso"):
                ...
                for linha in data.split("\n"):
                    if "ENDESC" in linha:
                        elemento = linha.split("=")
                        resp_desautoriza = Fiberhome.retornaResposta(elemento[2])
                    ...
                    if(physical_address == cliente['physical_address']):
                        ...
                        while (resp_autoriza != "Sucesso"):
                            ...
                            for linha in data.split("\n"):
                                ...
                                if "ENDESC" in linha:
                                    elemento = linha.split("=")
                                    resp_autoriza = Fiberhome.retornaResposta(elemento[2])
